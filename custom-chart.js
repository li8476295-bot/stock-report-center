...
(function (global) {
  ...
  function drawNav(canvas, sim, exp, title) {
    ...
    var all = {}; (sim || []).forEach(function (x) { all[x[0]] = x[1]; });
    (exp || []).forEach(function (x) { all[x[0]] = x[1]; });
    ...
    if (sim && sim.length) line(sim, COL.up, []);
    if (exp && exp.length) line(exp, '#c792ea', [6, 4]);
    drawText(ctx, title || '资金净值（基准100）', padL, 16, COL.title, 12);
    drawText(ctx, '■ 模拟盘', padL + 200, 16, COL.up, 10);
    drawText(ctx, '◇ 策略期望', padL + 260, 16, '#c792ea', 10);
  }
